...
contract ThePackage is ICar {
    ...
    function randomNumbaBaby(
        Monaco.CarData memory ourCar,
        Monaco.CarData calldata opps1,
        Monaco.CarData calldata opps2
    ...
    function boost(
        ...
        for (i; i < MAX_DELTA; i++) {
            boostToBuy = amount - i;
            if (
                monaco.getAccelerateCost(boostToBuy) < (car.balance / threshold)
            ) {
        ...
        if (!boosted && monaco.getAccelerateCost(1) <= (car.balance * 66) / 100)
            monaco.buyAcceleration(1);
    }

    // Wraps monaco.buyAcceleration() with some economy checks
    function shell(Monaco monaco, Monaco.CarData memory car) private {
    ...
    // if so, stop 'em
    function oppStopper(
        Monaco.CarData calldata car,
        Monaco.CarData calldata opps
        ...
        if (
            ...
            (car.speed < opps.speed) &&
            (DELTA_LIMITER < (opps.speed - car.speed))
        ) {
            return true;
        } else {
            return false;
    ...
    function takeYourTurn(
        ...
        if (MID_GAME < car.y) boostCounter += max_bid(monaco, car);
        ...
        if (
            car.y < MID_GAME &&
            ourCarIndex == 0 &&
            randomNumbaBaby(car, allCars[0], allCars[1]) < 50
        ) {
            return;
        ...
        if (MADMAX < car.y && ourCarIndex != 0) {
            toShell =
                toShell ||
                madMaxShelling(monaco, car, allCars[ourCarIndex - 1]);
        }

        (uint256 _boost, bool _toShell) = doLogic(
            monaco,
            allCars,
            ourCarIndex,
            eco
        ...
        uint256 lagSpeed;
        if (ourCarIndex == 0) {
            lagSpeed = allCars[1].speed < allCars[2].speed
                ? allCars[2].speed
                : allCars[1].speed;
        ...
        if (toShell && lagSpeed < LAGGING_SPEED_DEMON) shell(monaco, car);
        else if (oppMustBeStopped) shell(monaco, car);

        if (0 < boostCounter) boost(monaco, car, boostCounter);
    }
    ...
    function decision(
        uint256 ourCarIndex,
        Monaco.CarData calldata car,
        Monaco.CarData calldata firstCar,
        Monaco.CarData calldata secondCar,
        Monaco.CarData calldata thirdCar,
        GapType eco
        ...
        if (ourCarIndex == 0) {
            if (car.y < 600) {
                ...
                gap == GapType.Large &&
                getGap(secondCar, thirdCar) == GapType.Small
            ) {
            ...
            // got money to shell
            if (
                eco == GapType.Large || getDelta(car, firstCar) == GapType.Large
            ) {
                toShell = true;
            }
            if (getGap(car, firstCar) == GapType.Small) {
                if (FLATOUT <= car.y) moreBoost += drs(car, firstCar, 2, 1);
                if (FLATOUT > car.y) moreBoost += drs(car, firstCar, 0, 0);
            } else if (
                MID_GAME < car.y && getGap(car, firstCar) != GapType.Small
            ) {
                ...
                if (FLATOUT <= car.y) moreBoost += drs(car, firstCar, 2, 2);
                if (FLATOUT > car.y) moreBoost += drs(car, firstCar, 1, 0);
                if (rng < 50 && !toShell) {
                    toShell = true;
                }
            }
            ...
            if (car.y < MID_GAME) {
                ...
                if (MADMAX <= car.y && car.y < FLATOUT)
                    moreBoost += drs(car, secondCar, 0, 0);
                if (FLATOUT <= car.y) moreBoost += drs(car, secondCar, 2, 2);
    ...
    function doLogic(
        ...
        uint256 ourCarIndex,
        GapType eco
    ) private view returns (uint256 boostCounter, bool toShell) {
        Monaco.CarData calldata car = allCars[ourCarIndex];
        ...
        (_boost, _shell) = checkBurn(monaco, ourCarIndex, car, toShell);
        boostCounter += _boost;
        toShell = toShell || _shell;

        // if we're in the late game, and opps cannot afford shells, then boost
        if (MADMAX <= car.y) {
            (_boost) = safeBoost(monaco, allCars, ourCarIndex);
            boostCounter += _boost;
        }
    }
    ...
    function checkBurn(
        ...
        Monaco.CarData calldata car,
        bool toShell
    ) private view returns (uint256 _moreBoost, bool _toShell) {
        if (MID_GAME < car.y && car.y < MADMAX)
            (_moreBoost, _toShell) = excess_burn(
                monaco,
                car,
                ourCarIndex,
                1,
    ...
    function safeBoost(
        ...
        Monaco.CarData[] calldata allCars,
        uint256 ourCarIndex
    ) private view returns (uint256 _moreBoost) {
    ...
    function drs(
        Monaco.CarData calldata car,
        Monaco.CarData calldata opp,
        uint256 delta,
        uint256 fasterCase
    ) private pure returns (uint256 moreBoost) {
        moreBoost = (opp.speed < car.speed)
            ? fasterCase
            : (opp.speed + delta - car.speed);
    }

    // determines if theres excess cash to spend
    function excess_burn(
        Monaco monaco,
        Monaco.CarData calldata car,
        uint256 place,
        uint256 multiplier,
        bool shelled
    ) private view returns (uint256 moreBoost, bool toShell) {
    ...
    function max_bid(
        Monaco monaco,
        Monaco.CarData calldata car
    ) private returns (uint256 moreBoost) {
        // if its cheap, fuggit
    ...
    function getEco(
        Monaco.CarData calldata car,
        Monaco.CarData calldata opp1,
        Monaco.CarData calldata opp2
    ) private pure returns (GapType) {
