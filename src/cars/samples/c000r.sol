...
contract c000r is ICar {
    function takeYourTurn(
        ...
        if (
            ...
            allCars[0].y + allCars[0].speed >= 1000
        ) {
        ...
        if (allCars[0].y > 850) {
            ...
            if (
                (ourCarIndex == 0 && (allCars[1].speed > ourCar.speed)) ||
                allCars[2].speed > ourCar.speed
            ) {
                uint256 largerSpeed = allCars[1].speed > allCars[2].speed
                    ? allCars[1].speed
                    : allCars[2].speed;
                if (
                    ourCar.balance >=
                    monaco.getAccelerateCost(largerSpeed - ourCar.speed)
                ) {
            ...
            if (ourCarIndex != 0) {
                ...
                if (
                    ourCar.balance >= shellCost &&
                    shellCost < 2000 &&
                    ((frontCarSpeed > ourCar.speed && frontCarSpeed > 4) ||
                        frontCarSpeed > 8)
                ) {
                    ourCar.balance -= uint24(monaco.buyShell(1));
                    shellCost = 15001;
        ...
        if (ourCarIndex == 2) {
            if (
                allCars[1].balance < 200 ||
                (allCars[0].y > 825 && allCars[1].y < 750)
            ) {
                // try and get into second place in order to shell next turn.
                costToCatchUp = monaco.getAccelerateCost(
                    1 +
                        allCars[1].speed +
                        allCars[1].y -
                        (ourCar.speed + ourCar.y)
                );
        ...
        if (
            ourCarIndex == 1 && nextCarSpeed > 3 && ourCar.balance >= shellCost
        ) {
            // if the car ahead of us is going fast and it's not really expensive to get em, smoke em.
            if (
                shellCost < 300 &&
                (nextCarSpeed > (ourCar.speed + 6) || nextCarSpeed > 24)
            ) {
                ourCar.balance -= uint24(monaco.buyShell(1));
            } else if (
                shellCost < 1000 &&
                (nextCarSpeed > 35 ||
                    allCars[0].y - ourCar.y > 50 ||
                    allCars[0].y > 950 ||
                    ourCar.balance > 1000)
            ) {
        ...
        if (
            ...
            try monaco.buyAcceleration(ourCarIndex == 0 ? 2 : 4) returns (
                uint256 cost
            ) {
            ...
            uint256 catchUpAccelerationAmount = (allCars[1].speed -
                ourCar.speed) / (costToCatchUp < 50 ? 1 : 2);
            ourCar.balance -= uint24(
                monaco.buyAcceleration(catchUpAccelerationAmount)
            );
            ...
            uint256 costToAccelerate = monaco.getAccelerateCost(
                accelerationTarget
            );
            ...
            if (
                ...
                ourCar.balance -= uint24(
                    monaco.buyAcceleration(accelerationTarget)
                );
        ...
        if (
            ourCarIndex != 0 &&
            allCars[0].y > 700 &&
            ourCar.balance > allCars[0].balance * 2 &&
            ourCar.balance > 220
        ) {
            try monaco.buyAcceleration(5) {} catch {}
        }
