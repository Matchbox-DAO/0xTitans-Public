...
contract RunRace is Script {
    ...
    function setUp() public {
        string memory car1File = vm.readFile("./race/car1.bytecode");
        string memory car2File = vm.readFile("./race/car2.bytecode");
        string memory car3File = vm.readFile("./race/car3.bytecode");
        ...
        bytes memory bCar1Arged = abi.encodePacked(vm.parseBytes(car1File));
        bytes memory bCar2Arged = abi.encodePacked(vm.parseBytes(car2File));
        bytes memory bCar3Arged = abi.encodePacked(vm.parseBytes(car3File));

        assembly {
            addr1 := create(0, add(bCar1Arged, 0x20), mload(bCar1Arged))
            addr2 := create(0, add(bCar2Arged, 0x20), mload(bCar2Arged))
            addr3 := create(0, add(bCar3Arged, 0x20), mload(bCar3Arged))
        }

        require(addr1 != address(0) && addr2 != address(0) && addr3 != address(0), "StdCheats deployCode(string,bytes): Deployment failed.");
